import type { EquipmentType } from '@/game/character/types';

export type InventoryRarity = 'common' | 'uncommon' | 'rare' | 'epic';
export type InventoryActionType = 'equippable' | 'usable' | 'none';
...
export interface InventoryStatLine {
  label: string;
  value: string;
}
...
export interface InventoryItem {
  ...
  quantity: number;
  description: string;
  stats: InventoryStatLine[];
  bonuses: InventoryStatLine[];
  powers: string[];
  attributes: string[];
  effects: string[];
  equipSlot?: EquipmentType;
  useEffect?: InventoryUseEffect;
}
...
export function createInventoryItemById(itemId: string): InventoryItem | null {
  const item = INVENTORY_ITEMS.find((entry) => entry.id === itemId);
  return item ? { ...item } : null;
}

export function createWorldPickupTextureKey(itemId: string): string {
